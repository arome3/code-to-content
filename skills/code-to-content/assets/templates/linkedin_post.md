# LinkedIn Post Template

Instructions: The first 2 lines appear before "See more" - make them count! Delete instructions when done.

---

[HOOK LINE 1: Surprising statement, result, or confession]

[HOOK LINE 2: Build intrigue, setup the story]

---

[STORY PARAGRAPH 1: Set the scene]
[When/where did this happen? What was the situation? Be specific—names, dates, companies make it real.]

[STORY PARAGRAPH 2: The challenge/conflict]
[What problem arose? What was at stake? What was the tension?]

[STORY PARAGRAPH 3: What you did — and what you chose NOT to do]
[What action did you take? What decision did you make? Name the obvious alternative you rejected and why (Roads Not Taken from the Differentiation Brief) — the choice is the story.]

[STORY PARAGRAPH 4: The outcome]
[What was the result? How did it turn out? Include specific numbers if possible.]

---

[OPTIONAL — THE SPIKY PART: One sentence stating the unpopular or non-obvious thing you now believe. The line a competitor wouldn't sign. Skip only if the Differentiation Brief captured no opinion.]

Here's what I learned:

• [Insight 1: Actionable takeaway]

• [Insight 2: Actionable takeaway]

• [Insight 3: Actionable takeaway]

---

[ENGAGEMENT QUESTION: Genuine question that invites discussion]

#[Hashtag1] #[Hashtag2] #[Hashtag3]

---

## Character Count Guide

- Hook (visible): ~200 characters
- Full post: 1,300-3,000 characters (sweet spot: ~1,500)
- Hashtags: 3-5 relevant ones

---

## LinkedIn Best Practices

- [ ] First 2 lines are compelling (before "See more")
- [ ] Personal story, not generic advice
- [ ] Specific details (numbers, names, dates)
- [ ] Line breaks for readability
- [ ] Business/career relevance clear
- [ ] Question invites genuine discussion
- [ ] 3-5 relevant hashtags
- [ ] No external links in main text (hurts reach)
- [ ] Post link in first comment instead

---

## Content Angles for Dev/Tech

- Career growth story
- Technical decision and its impact
- Leadership/management lesson
- Failure and what you learned
- Team achievement
- Industry insight/prediction
- Hiring/interviewing experience
- Build in public update

---

## Copy-Ready Output Format

When generating final content, provide in this format:

```
---COPY BELOW FOR LINKEDIN---

[Hook line 1 - visible before "See more"]

[Hook line 2 - visible before "See more"]

[Story paragraph 1]

[Story paragraph 2]

[Story paragraph 3]

[Story paragraph 4]

Here's what I learned:

• [Insight 1]

• [Insight 2]

• [Insight 3]

[Engagement question]

#hashtag1 #hashtag2 #hashtag3

---END COPY---

Character Count: XXXX/3000 ✓
Hook (first 200 chars): ✓ Compelling
Hashtags: X relevant tags

First Comment (post separately):
[Link to resource if any]

Best Posting Time: [day] [time] [timezone]
```

This format ensures easy copying to LinkedIn and validates all limits.
